    ...
    public class ClientService : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            foreach (var repos in grouppedRepos)
            {
                ...
                var reposResponse = JsonSerializer.Deserialize<List<RepoResponse>>(await response.Content.ReadAsStringAsync(), _serializerOptions);
                ...
                foreach (var repoWithMissingFiles in reposWithMissingFiles)
                {
                    ...
                    foreach (var missingFile in repoWithMissingFiles.LfsFileNames)
                    {
                        ...
                        var savePath = Path.Combine(localRepo.Path, missingFile.Substring(0, 2), missingFile.Substring(2, 2), missingFile);
